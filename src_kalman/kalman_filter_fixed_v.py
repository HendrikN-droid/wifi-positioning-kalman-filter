...
def f(x):
    return np.array(
        [
            x[0] + config["v_x"] * config["dt"],
            x[1] + config["v_y"] * config["dt"],
        ]
    )
...
def e_kalman_filter_2(x, P, measurements):
    estimated_positions = []
...
if __name__ == "__main__":
    ...
    x_ini = np.array([1.5, 6.5])  # [x, y]
    P_ini = np.eye(2)  # Initial covariance

    estimated_positions = e_kalman_filter_2(
